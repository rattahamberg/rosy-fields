...
const checks = [
  ...
  ["ledger indexes", `SELECT indexname FROM pg_indexes WHERE schemaname='public' AND indexname IN ('expense_household_spent_at_idx', 'expense_household_active_idx', 'expense_paid_by_idx', 'expense_split_user_id_idx', 'settlement_household_settled_at_idx', 'settlement_household_active_idx', 'household_audit_log_household_idx') ORDER BY indexname`],
