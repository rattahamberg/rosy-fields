...
- **Use `peekSession` ONLY when an absent session should render gracefully** (header chrome, footer email, etc.). Everything else uses `verifySession` or `verifyAdmin`.
- Session DAL: `lib/dal.ts` exposes:
  - `peekSession()` — returns the session or `null`, never redirects.
...
- The `proxy.ts` matcher only checks for session-cookie *presence* — it's a UX/perf hint, never the authorization boundary. Don't try to "improve" it to do real auth; that work belongs in the DAL.
...
- **For every `CREATE INDEX` in a generated migration: add `IF NOT EXISTS` by hand.** drizzle-kit emits unconditional CREATE INDEX, which fails on re-application against a DB where the index already exists (e.g. after a manual hotfix or environment drift). Same for `CREATE EXTENSION` and `ADD COLUMN` where idempotency matters.
- Apply locally with `npm run migrate`. CI applies prod automatically on push to master.
- The `__migrations` table tracks SHA-256 of each file. **Never edit a migration file after it's been applied** — `npm run migrate` will refuse to run if file content drifts from the stored hash. If you must edit (e.g. add IF NOT EXISTS retroactively), follow up with `npm run migrate:rehash`.
...
- **Derive Drizzle row types** from the query rather than hand-typing: `type Row = Awaited<ReturnType<typeof someQuery>>[number]` (see `UserListRow` in `lib/admin/queries.ts`). Hand-typed shapes drift silently when columns change.
